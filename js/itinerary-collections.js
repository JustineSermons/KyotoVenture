...
function displayItineraryDetails(itinerary) {
  ...
  itineraryTitle.textContent =
    itinerary.itinerary_name || "Itinerary Collection Name";
  ...
  document.getElementById("destinations").value = itinerary.destinations
  ...
  document.getElementById("startDate").value =
    startDate && !isNaN(startDate) ? startDate.toISOString().split("T")[0] : "";
...
function displayActivities(activities, totalDays) {
  ...
  for (let i = 1; i <= totalDays; i++) {
    ...
    const dayActivities = isCollectionEmpty
      ? []
      : activities.filter((activity) => activity.day === i);
    ...
    if (dayActivities.length === 0) {
      ...
      dayActivities.forEach((activity) => {
        ...
        activityContainer.setAttribute(
          "data-activity-id",
        ...
        img.src = activity.image_url
          ? activity.image_url
          : "assets/images/thetemplekyoto.png";
        ...
        tagTwo.textContent = activity.day
          ? `Day ${activity.day}`
          : "Unscheduled";
        ...
        moveButton.onclick = () =>
          showMoveActivityPopup(activity.activity_id, activity.day, totalDays);
  ...
  try {
    ...
    confirmButton.addEventListener("click", async () => {
      ...
      const activityElement = document.querySelector(
        `.itinerary-container[data-activity-id='${activityId}']`
      );
      if (activityElement) {
